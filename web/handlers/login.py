...
class LoginHandler(BaseHandler):
    ...
    async def post(self):
        ...
        async with self.db.transaction() as sql_session:
            ...
            regFlg = False if (await self.db.site.get(1, fields=('regEn',), sql_session=sql_session))['regEn'] == 0 else True
    ...
    async def send_mail(self, user, sql_session=None):
        ...
        await gen.convert_yielded(utils.send_mail(to=user['email'], subject=u"欢迎注册 定时平台", html=u"""
                ...
                <div style="margin:40px auto;width:90%%">
                    ...
                    <p>请注意：此邮件由 <a href="{http}://{domain}/verify/{code}" style="color:#12addb" target="_blank">定时平台</a> 自动发送，请勿直接回复。</p>
...
class PasswordResetHandler(BaseHandler):
    ...
    async def send_mail(self, user):
        ...
        await gen.convert_yielded(utils.send_mail(to=user['email'], subject=u"定时平台(%s) 密码重置" % (config.domain), html=u"""
