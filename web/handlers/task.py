    ...
    async def post(self, taskid):
        ...
        async with self.db.transaction() as sql_session:
            ...
            try:
                ...
                title = u"定时任务 {0}-{1} 失败".format(tpl['sitename'], task['note'])
                logtmp = u"{0} \\r\\n日志：{1}".format(t, e)
                ...
                await self.finish('<h1 class="alert alert-danger text-center">定时失败</h1><div class="showbut well autowrap" id="errmsg">%s<button class="btn hljs-button" data-clipboard-target="#errmsg" >复制</button></div>' % logtmp.replace('\\r\\n', '<br>'))
            ...
            t = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            title = u"定时任务 {0}-{1} 成功".format(tpl['sitename'], task['note'])
